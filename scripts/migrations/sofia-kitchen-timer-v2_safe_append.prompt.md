...
description: Sofia's kitchen timer — check pending_tasks.md every 30 minutes and act on anything that's ready. Migrated 2026-04-29 to use safe_append.py for memory-file appends (auto ER-sync via the in-write code path); Edit-remove and JSON-snapshot writes retain direct semantics with explicit cp -p ER mirror.
---
