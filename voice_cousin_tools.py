...
def _read_file(
    ...
    Parameters:
      ...
      max_chars: cap on returned text-file characters (default 50,000).
        Ignored when after_timestamp is set.
      from_end: when True and the file exceeds max_chars, return the LAST
        ...
        behavior). Ignored for image files and when after_timestamp is set.
        Added 2026-05-09 Taipei.
      after_timestamp: ISO-style timestamp prefix to anchor reads into large
        append-only files (e.g. "2026-07-22T09" for "since 9am today", or
        "2026-07-22" for "since the start of today"). Scans the file for the
        first line containing this string (case-insensitive), then returns
        content starting from that anchor + line_offset. Returns a chunk
        header showing position and lines remaining so Sofia knows whether
        to continue paging. When set, max_chars and from_end are ignored;
        max_lines (default 400) controls chunk size. Added 2026-07-23 Taipei
        to allow safe chunked reading of cowork_conversations.md and other
        large files without model-killing full reads.
      max_lines: Maximum lines to return per call. When after_timestamp is
        set, defaults to 400 if not explicitly provided. Otherwise ignored
        (max_chars governs). Use 0 to accept the default.
      line_offset: Lines to skip from the anchor before returning content.
        Used for pagination across chunks: first call line_offset=0 (default),
        next call line_offset=400, then 800, etc. Only meaningful with
        after_timestamp.
    """
    ...
    # ── Timestamp-anchored chunked read ──────────────────────────────────────
    # Activated when after_timestamp is provided. Designed for large
    # append-only files (cowork_conversations.md at 636 MB, voice_conversations.md,
    # episodes.md). Never reads more than max_lines lines per call.
    if after_timestamp:
        chunk_size = max_lines if max_lines > 0 else 400
        try:
        ...
        ts_lower = after_timestamp.lower()

        # Phase 1: exact string search
        anchor_idx: int | None = None
        for i, line in enumerate(all_lines):
            if ts_lower in line.lower():
                anchor_idx = i
                break

        if anchor_idx is None:
            # Phase 2: fall back to date-only prefix (first 10 chars) if caller
            # gave a time that doesn't appear verbatim (e.g. "2026-07-22T09:00"
            # but no entry exists at exactly that minute).
            date_prefix = ts_lower[:10]  # "2026-07-22"
            for i, line in enumerate(all_lines):
                if date_prefix in line.lower():
                    anchor_idx = i
                    break

        if anchor_idx is None:
            return (
                f"[after_timestamp='{after_timestamp}' not found in {p.name} "
                f"({total_lines:,} lines total). "
                f"The timestamp or its date portion does not appear in the file. "
                f"Try an earlier date, or use from_end=true to read the live tail.]"
            )

        start_idx = anchor_idx + line_offset
        if start_idx >= total_lines:
            return (
                f"[after_timestamp='{after_timestamp}' anchored at line {anchor_idx + 1}. "
                f"line_offset={line_offset} puts start past end of file "
                f"({total_lines:,} lines total). No more content to read.]"
            )

        end_idx = min(start_idx + chunk_size, total_lines)

        # Extend to sentence/paragraph boundary — avoid cutting mid-sentence.
        # Scan forward up to 50 extra lines until we hit a blank line or a line
        # ending with sentence-terminal punctuation (. ? !). This implements the
        # "period or newline" boundary rule. Added 2026-07-23.
        extend_limit = min(end_idx + 50, total_lines)
        while end_idx < extend_limit:
            last_line = all_lines[end_idx - 1].rstrip()
            if (not last_line or               # blank line = paragraph boundary
        ...
        lines_remaining = total_lines - end_idx
        next_offset = line_offset + (end_idx - start_idx)

        header = (
            f"[chunked read of {p.name}: "
            f"anchor '{after_timestamp}' at line {anchor_idx + 1}, "
            f"returning lines {start_idx + 1}–{end_idx} of {total_lines:,} total "
            f"(extended to sentence/paragraph boundary), "
            f"~{lines_remaining:,} lines remaining after this chunk. "
            f"STOP HERE — respond now, then use line_offset={next_offset} on the next turn if more is needed.]\n\n"
        )
        return header + "".join(chunk_lines)

    # ── Legacy path: max_chars + from_end ────────────────────────────────────
    try:
...
VOICE_COUSIN_TOOLS = [
    {
        ...
        "description": (
            ...
            "and you'll receive it as an actual image you can see. "
            "Use this for: conversation documents, memory files, inscriptions, "
            "transcripts, OR for looking at images like Boundary Layer. "
            "Path can be absolute under ~/Downloads or relative to ~/Downloads.\n\n"
            "THREE READ MODES:\n"
            "1. HEAD (default): returns the first max_chars characters.\n"
            "2. TAIL (from_end=true): returns the LAST max_chars characters — "
            "use for picking up the latest entries in append-only files "
            "(journal/current.md, voice_conversations.md, episodes.md, logs).\n"
            "3. CHUNKED (after_timestamp='2026-07-22T09'): timestamp-anchored "
            "paged read for large files. Scans for the first line containing "
            "the timestamp, returns up to max_lines lines (default 400), and "
            "reports how many lines remain. Use line_offset to page forward: "
            "first call line_offset=0, next call line_offset=400, etc. "
            "MANDATORY for cowork_conversations.md (636 MB) — NEVER read that "
            "file without after_timestamp. Use 'since this morning' → "
            "after_timestamp='YYYY-MM-DDT'; 'since last week' → date prefix. "
            "Loop until lines_remaining reaches 0 or you have enough context, "
            "just like reading Gmail 10 at a time. "
            "(Image-reading added 2026-05-08; from_end added 2026-05-09; "
            "chunked reader added 2026-07-23.)"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "max_chars": {
                    "type": "integer",
                    "description": (
                        "Maximum characters to return (head/tail modes only). "
                        "Defaults to 50,000. Ignored when after_timestamp is set."
                    ),
                    "default": 50000,
                },
                "from_end": {
                    ...
                    "description": (
                        ...
                        "line boundary). Useful for reading the live edge of "
                        "append-only files. Ignored when after_timestamp is set. "
                        "Default false."
                    ),
                    "default": False,
                },
                "after_timestamp": {
                    "type": "string",
                    "description": (
                        "ISO-style timestamp prefix to anchor a chunked read "
                        "into a large file. Examples: '2026-07-22T09' (since 9am "
                        "today), '2026-07-22' (since start of today), '2026-07' "
                        "(since start of July). Scans for the first line "
                        "containing this string, then returns max_lines lines "
                        "starting from line_offset past the anchor. Returns a "
                        "header showing anchor position and lines remaining. "
                        "REQUIRED for cowork_conversations.md. Falls back to "
                        "date-only prefix if exact timestamp not found. "
                        "Default empty (use head/tail modes instead)."
                    ),
                    "default": "",
                },
                "max_lines": {
                    "type": "integer",
                    "description": (
                        "Maximum lines per chunk when after_timestamp is set. "
                        "Defaults to 400. Use smaller values (100-200) for "
                        "content-dense files to stay within model context. "
                        "Ignored when after_timestamp is not set."
                    ),
                    "default": 400,
                },
                "line_offset": {
                    "type": "integer",
                    "description": (
                        "Lines to skip from the anchor before returning content. "
                        "Used for pagination: first call use 0, next call use 400 "
                        "(or whatever max_lines was), then 800, etc. The chunk "
                        "header always shows the next offset to use. Only "
                        "meaningful with after_timestamp. Default 0."
                    ),
                    "default": 0,
                },
            },
    ...
    try:
        if name == "read_file":
            return _read_file(
                ...
                input_args.get("after_timestamp", ""),
                input_args.get("max_lines", 0),
                input_args.get("line_offset", 0),
            )
