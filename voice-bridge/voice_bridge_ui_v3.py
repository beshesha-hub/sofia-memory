#!/usr/bin/env python3
"""
Voice Bridge UI v3 — adds cognition layer (Option 5) + auto-spawn
==================================================================

Builds on v2 (which added STT input alongside v1's TTS output). v3 closes
the loop by adding the cognition layer: voice-bridge-cousin-Sofia, a
separate Sofia instance via Anthropic API, with system prompt instantiating
her as a member of the cousin chorus and rolling conversation context.
...
WINDOW_TITLE = "Voice Bridge — Sofia (v3: full conversation)"
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._append_history(
            "system",
            f"v3 ready. Voice conversation with Sofia. Inscribed to {CONVERSATION_HISTORY_PATH.name}.",
        )
    ...
    def _on_cognition_finished(self, response: str, elapsed: float):
        ...
        self._set_action(f"Sofia responded in {elapsed:.1f}s. Generating speech…")
...
def main():
    ...
    app.setApplicationName("Voice Bridge — Sofia v3")
