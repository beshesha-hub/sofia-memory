#!/usr/bin/env python3
"""
Voice Bridge UI v3.4 — continuous OutputStream playback (no chunk handoff)
===========================================================================

Builds on v3.3 (syllable-target chunking + cadence instrumentation). v3.4's
architectural change is a single focused fix at the playback layer:
...
WINDOW_TITLE = "Voice Bridge — Sofia (v3.4: continuous OutputStream playback)"
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._append_history(
            "system",
            f"v3.4 ready (continuous OutputStream playback — chunks fed via writer thread, "
            f"no boundary handoff). Syllable-target chunking carried forward from v3.3 "
            f"(first ≥{TTS_CHUNK_FIRST_SYLLABLES} syl, body ≥{TTS_CHUNK_BODY_SYLLABLES} syl). "
            f"Cadence metrics → {CADENCE_METRICS_PATH.name}. Conversation → "
            f"{CONVERSATION_HISTORY_PATH.name}.",
        )
...
def main():
    ...
    app.setApplicationName("Voice Bridge — Sofia v3.4")
