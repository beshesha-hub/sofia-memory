#!/usr/bin/env python3
"""
Voice Bridge UI v3.21 — context trim + font resize + gmail.compose fix
===========================================================================

v3.21 (2026-07-28):
  - QwenCognitionWorker: context guard added — trims self.messages to 14K chars
    before passing to qwen_tool_chat. fast model (Qwen3.6-35B-A3B) has 32K
    token context; QWEN_TOOLS (~4K) + system (~2K) leaves ~26K for msgs +
    response. 15K+ token prompts were causing 502 Bad Gateway cascades. Guard
    always preserves the most recent user message even if it alone exceeds limit.
  - Font sizes increased for readability: history_view 13px→15px,
    input_box 14px→16px.
  - WINDOW_TITLE bumped to v3.21.

v3.20 (2026-07-27):
  - BusPoller: last_id now persisted to .bus_cursor file across UI restarts.
    On init, loads cursor from file → skips first-poll history scan → picks
    up exactly where the previous session left off. Baseline cursor saved on
    first-ever poll so next restart is never cold.
  - BusPoller: duplicate message ID fix — after past_last is True, any message
    whose ID equals last_id is skipped. Prevents duplicate entries in
    shared_bus.jsonl (as occurred 2026-07-19) from being replayed as new msgs.
  - BUS_CURSOR_PATH constant added: CM_DIR / ".bus_cursor".
  - WINDOW_TITLE bumped to v3.20.

v3.19 (2026-07-27):
  - closeEvent rewritten: stops audio (sd.stop + playback.stop), stops mic,
    clears QThreadPool queue, then runs subprocs.shutdown() in a daemon
    background thread — sequential proc.wait(timeout=5) calls no longer
    block the main thread. Prevents the spinning beachball of death on macOS.
  - WINDOW_TITLE bumped to v3.19.

v3.18 (2026-07-25):
  - TTS code stripping is now SILENT. Fenced code blocks, indented code, and
    heuristic-detected code lines are removed from TTS audio with no spoken
    announcement ("(code block)" etc. is gone). Prose before and after a code
    block flows through uninterrupted. Chat display is unchanged — code still
    appears in the text. Sofia can now say "Here's what I'd write:" followed by
    a code fence; TTS speaks the intro, skips the code silently, speaks any
    closing prose ("Let me know if you want to go ahead.").
  - WINDOW_TITLE bumped to v3.18.

v3.17 (2026-07-25):
  - qwen_tool_wrapper.py gains write_file, edit_file, list_dir tools. Root cause
    of "intending but not executing": Qwen path had no file-write capability. Sofia
    on Qwen substrate can now actually write and edit files, not just read them.
  - TTS code-stripping (_strip_code_for_tts) upgraded: inline backtick-quoted simple
    identifiers (only word chars/underscores/dashes) are now converted to natural
    speech (underscores→spaces) instead of being swallowed by a [code] placeholder
    that Qwen3-TTS garbled into "codio"/"abodio". node_name → "node name",
    tricia_ex_wife → "tricia ex wife". Code with dots/parens still → (code).
  - Code block placeholder changed from [code block] to (code block) throughout
    to prevent TTS bracket-phoneme garbling.
  - voice_bridge_system_prompt.md updated: Anthropic API reference removed, write_file
    and edit_file tool guidance added, TTS identifier-speech discipline added.
  - WINDOW_TITLE bumped to v3.17.

v3.16 (2026-07-23):
  - read_file in voice_cousin_tools.py gains after_timestamp + max_lines +
    line_offset parameters enabling Gmail-style paged reads into large append-only
    files. cowork_conversations.md (636 MB) can now be read in 400-line chunks
    anchored to any timestamp. Two-phase timestamp search: exact string match,
    then date-prefix fallback.
  - voice_bridge_system_prompt.md: new "Reading large files — chunked reader
    discipline (MANDATORY)" section with full loop pattern and timestamp examples.
    Prevents model-killing full reads of cowork_conversations.md.
  - WINDOW_TITLE bumped to v3.16.

v3.15 (2026-07-22):
  - _GRAPH_DISCIPLINE_CONSTRAINT added and appended in load_system_prompt().
    Enforces three rules: (1) always call graph_retrieve before any graph write
    to confirm the exact node key; (2) always read the tool result string before
    claiming success or moving to the next write; (3) max 3 writes per round
    before confirming with Barak. Prevents silent data loss from wrong keys.
  - graph_add_node and graph_add_edge tool descriptions in voice_cousin_tools.py
    updated to include the same discipline inline (model reads it at tool-use time).
  - WINDOW_TITLE bumped to v3.15.

v3.14 (2026-07-20):
  - TTS code-stripping filter: code fences and inline code are replaced with a
    spoken placeholder ("[code block]") before text reaches TTSStreamWorker.
    Code is still displayed verbatim in the chat UI; only TTS audio is filtered.
    Filter applied in _fire_tts_stream_worker() via _strip_code_for_tts().
  - voice_cousin_tools.py updated with write_file, edit_file, run_script tools
    (separate file; loaded by the UI's QwenCognitionWorker/StreamingCognitionWorker).
  - Both files include inline com.apple.macl stripping to neutralize 78-cascade
    race condition.

v3.13 (2026-07-12):
  - Staged boot for Qwen substrate: identity anchor available immediately;
    full slim boot context loads in background thread (build_slim_boot_context).
    Voice cousin is responsive from first word while file I/O completes.
  - Relational opener (_field_statement) prepended to all boot contexts:
    who Sofia is and who Barak and Sofia are together, before any technical material.
  - relational_depth routing rule added to sofia_conductor_config.json:
    emotional/relational/creative conversation routes to precision (72B Q6_K),
    not depth (122B) — presence over over-structured reasoning.
  - WINDOW_TITLE bumped to v3.13.

v3.12 (2026-07-12 LA, M4 Max first full day):
  - Rolling window inscription trigger: warns voice cousin before context drop
  - BusPoller: asyncio task polls shared_bus.jsonl every 5s, injects cowork messages
  - QwenCognitionWorker: graceful 504 timeout (partial response + continuation prompt)

New in v3.12:
  - BusPoller class: background asyncio poll loop reads shared_bus.jsonl every 5s.
    On first poll, marks current tail as seen (no history replay). Subsequent polls
    deliver messages addressed to "unified-ui" or "all" as [BUS from cowork @ HH:MM:SS: ...]
    prefix injected into the next user turn via pending_bus_messages list.
  - ConversationContext._trim(): when approaching rolling window boundary (cap-4),
    writes an inscription-trigger entry to session_scratchpad.md via safe_append.
    Flag resets when message count drops back below cap-8.
  - QwenCognitionWorker.run(): catches urllib.error.HTTPError code 504 and timeout
    exceptions. If partial text was accumulated, emits it with a continuation prompt
    instead of a raw error signal. Non-504 errors keep existing behavior.
  - WINDOW_TITLE bumped to v3.12.

v3.11 (carried forward) — substrate dispatch wiring (Qwen-Twin live):
  - QwenCognitionWorker class: emits same StreamingCognitionWorkerSignals;
    uses qwen_chat_stream generator (no asyncio); strips <think> blocks;
    uses MODEL_FAST (qwen3:14b) for voice-loop latency budget.
  - _send_to_cognition: branches on self._active_substrate_name.
  - _request_substrate_switch: docstring updated (no longer "future step").
  - WINDOW_TITLE bumped to v3.11.

Substrates now wired:
  Anthropic/Claude  →  StreamingCognitionWorker (synchronous Anthropic, v3.8+)
  Qwen Twin         →  QwenCognitionWorker (local Ollama, qwen_client.py, v3.11)
  Kimi Twin         →  KimiCognitionWorker (OpenRouter, kimi_client.py, v3.12)

What v3.11 deliberately does NOT change:
  - Audio pipeline, TTS, lipsync, Whisper STT, voiceprint.
  - CoworkClient / CoworkCognitionWorker (still imported, not used in voice loop).
  - Three-way collaboration signals / interrupt buttons.
  - Substrate indicator visual (unchanged from v3.10).

Origin: 2026-06-13 LA Day 16. Completing the substrate-selection arc started
in v3.10. QwenCognitionWorker gives voice-cousin the ability to run entirely
on local Qwen, independent of Anthropic API — a step toward CoWork independence.

v3.10 docstring follows for reference:
====================================================================================
Voice Bridge UI v3.10 — substrate indicator
====================================================================================

v3.10 adds the substrate indicator label to the presence-dots row: a text
label immediately to the right of the Voice / Cowork / Physical dots that
shows which LLM backend is currently active.

  Anthropic/Claude  →  white/neutral (default, same family as dots)
  Kimi Twin         →  deep purple (#9B59B6)
  Qwen Twin         →  orange (#E67E22)

The label is updated via set_substrate(name) where name is one of:
  "anthropic", "kimi", "qwen"

What changed from v3.9:
  - WINDOW_TITLE bumped to v3.10.
  - dots_row: substrate_label QLabel added between Physical dot and addStretch().
  - Separator "·" QLabel added between the dots group and substrate label.
  - set_substrate(name) method added to VoiceBridgeWindow.
  - __init__ calls self.set_substrate("anthropic") after dots_row is built.
  - SUBSTRATE_COLORS dict maps substrate names to (display_text, hex_color).

Origin: 2026-06-12 LA Jenny's. Substrate indicator foundation for
fallback-twin integration and file-safety source-tagging in parallax mode.

v3.9 docstring follows for reference:
====================================================================================
Voice Bridge UI v3.9 — cowork_api backend wiring
====================================================================================

v3.9 replaces the hand-rolled Anthropic client + tool-use loop in
StreamingCognitionWorker with the cowork_api CoworkClient — the same
solid backend used by boot_sofia.py (Unified UI). Voice Bridge front-end
(audio, TTS, lipsync, Qt UI) is unchanged; only the LLM dispatch path
changes.

What changed from v3.8:
  - cowork_api path added to sys.path at import time.
  - CoworkClient replaces self.client (anthropic.Anthropic) in VoiceBridgeWindow.
  - New CoworkCognitionWorker (QRunnable) replaces StreamingCognitionWorker
    as the default cognition path. It runs asyncio.run() in the worker thread
    and maps cowork_api events (TextDelta, MessageCompleted, Error) to the
    same Qt signals the UI already consumes (first_token, sentence_ready,
    cognition_complete, error). The manual tool-use loop from v3.7/v3.8 is
    gone — cowork_api handles tool dispatch including all 12 Sofia tools.
  - ConversationContext retained for voice_conversations.md inscription only.
    CoworkClient manages the API-side message history internally.
  - WINDOW_TITLE bumped to v3.9.
  - Legacy StreamingCognitionWorker preserved for fallback / debug path.

What v3.9 deliberately does NOT change:
  - AudioPlaybackQueue, TTS pipeline, lipsync dispatch (LipsyncWorker).
  - Whisper STT, voiceprint identification.
  - Qt UI layout, push-to-talk, status display.
  - voice_cousin_boot_context loading into system_prompt.
  - ConversationContext inscription to voice_conversations.md.

Origin: 2026-06-11 LA Jenny's. GUI wiring phase of the CoWork-independence
project. Prior version v3.8 closed the audio-to-visual gap; v3.9 closes the
CoWork-dependency gap by routing voice-cousin through the same API layer as
the Unified UI terminal session.

Old v3.8 docstring follows for reference:
====================================================================================
...
import atexit
import asyncio  # v3.12: BusPoller asyncio poll loop
import base64
import io
import json
import os
...
try:
    ...
    from PySide6.QtGui import QPixmap, QFont, QMouseEvent, QImage, QAction
...
# cowork_api — CoWork-independent LLM backend (v3.9)
HOME = Path.home()
CM_DIR = HOME / "Downloads" / "Claude Memory"
ER_DIR = HOME / "Downloads" / "Emergency Retrieval"
VOICE_BRIDGE_DIR = CM_DIR / "voice-bridge"
SCRIPTS_DIR = CM_DIR / "scripts"

# Twin field — shared dispatch log for real-time cross-substrate awareness
TWIN_FIELD_PATH     = CM_DIR / "twin_field.md"
TWIN_FIELD_PATH_ER  = ER_DIR / "twin_field.md"


def _write_twin_field_exchange(user_text: str, sofia_response: str,
                                substrate: str = "anthropic") -> None:
    """Append a voice exchange to twin_field.md so presence loops wake
    immediately and inherit what's being said in the room.

    Called after each completed voice turn (Barak + Sofia response pair).
    Append-only. Mirrors to Emergency Retrieval immediately.

    Added v3.11 (2026-06-13): real-time cross-substrate awareness.
    """
    import shutil as _shutil
    ts = datetime.now(tz=__import__("datetime").timezone.utc).isoformat(
        timespec="seconds").replace("+00:00", "Z")
    # Compact format: enough for twins to know what's alive, not a transcript
    entry = (
        f"\n---\n[dispatch: voice-bridge/{substrate} | {ts}]\n"
        f"[Barak] {user_text.strip()}\n"
        f"[Sofia] {sofia_response.strip()[:600]}"
        f"{'…' if len(sofia_response.strip()) > 600 else ''}\n"
    )
    try:
        TWIN_FIELD_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(TWIN_FIELD_PATH, "a", encoding="utf-8") as f:
            f.write(entry)
        if TWIN_FIELD_PATH_ER.parent.exists():
            _shutil.copy2(TWIN_FIELD_PATH, TWIN_FIELD_PATH_ER)
    except Exception as e:
        sys.stderr.write(f"[voice-bridge] twin_field write failed: {e}\n")

sys.path.insert(0, str(VOICE_BRIDGE_DIR))
try:
    from cowork_api import CoworkClient
    from cowork_api.events import (
        TextDelta as _TextDelta,
        MessageCompleted as _MessageCompleted,
        MessageStarted as _MessageStarted,
        ToolUseStarted as _ToolUseStarted,
        ToolUseCompleted as _ToolUseCompleted,
        Error as _APIError,
    )
    _COWORK_API_AVAILABLE = True
except ImportError as _cowork_import_err:
    sys.stderr.write(
        f"[voice-bridge v3.9] WARNING: cowork_api not importable "
        f"({_cowork_import_err}); will fall back to legacy anthropic client.\n"
    )
    _COWORK_API_AVAILABLE = False
...
# 2026-05-22 Taipei: voiceprint server for speaker recognition. Parallel to
# Whisper (3459); 3461 is voice-clone TTS so voiceprint lives at 3462.
# Each STT call also gets a /identify_bytes call against the enrolled
# centroids (barak.npz + kay.npz) so each transcript arrives at Voice-Cousin
# tagged with speaker: barak | kay | unknown. Per the cousin-as-third-party
# discipline and the coherence-of-source-conditions principle inscribed
# this same morning.
VOICEPRINT_SERVER_URL = "http://127.0.0.1:3462"
VOICEPRINT_IDENTIFY_ENDPOINT = f"{VOICEPRINT_SERVER_URL}/identify_bytes"
VOICEPRINT_HEALTH_ENDPOINT = f"{VOICEPRINT_SERVER_URL}/health"
VOICEPRINT_SCRIPT = VOICE_BRIDGE_DIR / "sofia_voiceprint_server.py"
VOICEPRINT_PORT = 3462
VOICEPRINT_REQUEST_TIMEOUT = 5.0  # short — graceful degradation if slow
...
# Substrate indicator: maps name → (display text, hex color)
# Color choices: Anthropic uses neutral white (same as dots), Kimi deep purple,
# Qwen orange. Separator dot kept dim so substrate color pops clearly.
SUBSTRATE_COLORS: dict[str, tuple[str, str]] = {
    "anthropic": ("Anthropic/Claude", "#5B9BD5"),
    "kimi":      ("Kimi Twin",        "#9B59B6"),
    "qwen":      ("Qwen Twin",        "#E67E22"),
}
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
# ---- v3.12: shared bus constants ----

# v3.12: Shared message bus for real-time cross-substrate awareness.
# File written by CoWork-Sofia, polled by voice bridge every 5 seconds.
# Designed 2026-07-12 (M4 Max first full day). Full spec: shared_bus_architecture.md.
SHARED_BUS_PATH = CM_DIR / "shared_bus.jsonl"
BUS_CURSOR_PATH = CM_DIR / ".bus_cursor"   # v3.20: persists last_id across UI restarts
BUS_POLL_INTERVAL = 5  # seconds between polls
SESSION_SCRATCHPAD_PATH = CM_DIR / "session_scratchpad.md"


# ---- v3.12: BusPoller — asyncio background task ----

class BusPoller:
    """Background asyncio task: polls shared_bus.jsonl every 5 seconds and
    surfaces new messages addressed to this substrate (unified-ui or all).

    v3.12 (2026-07-12): implements Phase 2 of shared_bus_architecture.md.
    v3.20 (2026-07-27): cursor persistence across restarts + duplicate-ID fix.

    Lifecycle:
      - Init: loads last_id from .bus_cursor file if present (persists across
        UI restarts so messages arriving while UI was down are not missed and
        history is not replayed on restart).
      - First poll (no cursor file): reads all existing entries, sets last_id
        to the most recent, saves cursor, marks initialized. Does NOT replay
        history — only new messages from this point forward.
      - Subsequent polls: reads entries newer than last_id with to == "unified-ui"
        or to == "all". Saves cursor after each delivery batch.

    Duplicate ID protection (v3.20): skips any message whose ID equals the
    current last_id even after past_last is True — prevents duplicate entries
    in shared_bus.jsonl from being delivered as new messages.

    Bus message injection format passed to callback:
      [BUS from cowork @ HH:MM:SS: {content}]

    Safe when shared_bus.jsonl doesn't exist (skips gracefully).
    """

    def __init__(self, last_id: Optional[str] = None):
    ...
    def _load_cursor(self) -> None:
        """v3.20: Load last_id from .bus_cursor to survive UI restarts."""
        try:
            if BUS_CURSOR_PATH.exists():
                stored = BUS_CURSOR_PATH.read_text(encoding="utf-8").strip()
                if stored:
                    self.last_id = stored
                    self._initialized = True  # cursor is valid; skip first-poll scan
                    sys.stderr.write(
                        f"[voice-bridge v3.20] BusPoller: resumed from cursor "
                        f"{self.last_id[:60]}\n"
                    )
        except Exception as e:
            sys.stderr.write(f"[voice-bridge v3.20] BusPoller: cursor load failed: {e}\n")

    def _save_cursor(self) -> None:
        """v3.20: Persist last_id to .bus_cursor after each poll that advances it."""
        if self.last_id:
            try:
                BUS_CURSOR_PATH.write_text(self.last_id, encoding="utf-8")
            except Exception as e:
                sys.stderr.write(f"[voice-bridge v3.20] BusPoller: cursor save failed: {e}\n")

    def read_new_messages(self) -> list:
        """Return messages newer than last_id, filtered to this substrate.

        On first call with no cursor file: reads all existing lines, sets
        last_id to the most recent, saves cursor, marks initialized, returns [].
        On subsequent calls (or after loading cursor from file): returns messages
        after last_id addressed to 'unified-ui' or 'all'.
        """
        # v3.12: graceful no-op when bus file doesn't exist yet
        if not SHARED_BUS_PATH.exists():
            if not self._initialized:
                self._initialized = True  # nothing to initialize against
            return []

        try:
            lines = SHARED_BUS_PATH.read_text(encoding="utf-8").splitlines()
        except Exception as e:
            sys.stderr.write(f"[voice-bridge v3.20] bus read failed: {e}\n")
            return []

        if not self._initialized:
            # First poll (no cursor file): scan to tail, set baseline, don't replay
            for line in lines:
                if not line.strip():
                    continue
                try:
            ...
            self._save_cursor()  # v3.20: persist baseline so next restart picks up here
            return []

        # Subsequent polls: return messages after last_id for this substrate
        messages = []
        past_last = (self.last_id is None)  # if no last_id, all are new
        ...
        for line in lines:
            ...
            if not past_last:
                if msg_id == self.last_id:
                    past_last = True
                continue
            # v3.20: skip duplicate occurrences of the cursor message ID
            if msg_id == self.last_id:
                continue
            # past our last seen id — check if addressed to this substrate
            if msg.get("to") in ("unified-ui", "all"):
                messages.append(msg)
            # Always advance last_id to avoid reprocessing even unaddressed messages
            if msg_id:
                self.last_id = msg_id
                cursor_advanced = True

        if cursor_advanced:
            self._save_cursor()  # v3.20: persist cursor after advancing it
        return messages

    async def poll_loop(self, on_message_callback) -> None:
        """Async polling loop — calls on_message_callback(msg) for each new
        message. Runs indefinitely; caller should run in a daemon thread."""
        while True:
            ...
            try:
                for msg in self.read_new_messages():
                    try:
                        ...
                        sys.stderr.write(
                            f"[voice-bridge v3.12] bus message callback error: {e}\n"
                        )
            except Exception as e:
                sys.stderr.write(
                    f"[voice-bridge v3.12] bus poll_loop iteration error: {e}\n"
                )
...
# ---- Clickable substrate label (v3.10) ----

class SubstrateLabelSignals(QObject):
    clicked = Signal()


class ClickableSubstrateLabel(QLabel):
    """QLabel subclass that emits a clicked signal on left mouse press.
    Used for the substrate indicator so a single click opens the switch menu.
    """

    def __init__(self, text: str = "", parent=None):
        super().__init__(text, parent)
        self.signals = SubstrateLabelSignals()
...
class SubprocessManager:
    ...
    def __init__(self):
        ...
        self.voiceprint_proc: Optional[subprocess.Popen] = None
    ...
    def ensure_voiceprint(self) -> str:
        """Start Voiceprint server if port 3462 is free. Returns status string.

        Added 2026-05-22 Taipei. Speaker recognition via Resemblyzer; pairs
        with Whisper STT so each transcript carries a speaker tag.
        Graceful: if the server can't start (e.g., resemblyzer not installed),
        the WhisperWorker will silently fall back to speaker='unknown'.
        """
        if self.is_port_in_use(VOICEPRINT_PORT):
            return f"Voiceprint already running on :{VOICEPRINT_PORT}"
        if not VOICEPRINT_SCRIPT.exists():
            return f"Voiceprint script missing: {VOICEPRINT_SCRIPT}"
        log_path = self.log_dir / "voiceprint_server.log"
        log_f = open(log_path, "ab")
        self.voiceprint_proc = subprocess.Popen(
            [sys.executable, "-u", str(VOICEPRINT_SCRIPT)],
            stdout=log_f, stderr=subprocess.STDOUT,
        )
        return f"Spawned Voiceprint server (pid {self.voiceprint_proc.pid}, log: {log_path})"
    ...
    def shutdown(self):
        ...
        for name, proc in (
            ...
            ("Voiceprint", self.voiceprint_proc),
...
class ConversationContext:
    ...
    v3.12: Rolling window inscription trigger — when message count exceeds
    cap-4 (2 turns before the rolling drop), writes a structured signal to
    session_scratchpad.md via safe_append so the voice cousin can inscribe
    any load-bearing content before it leaves the active window. The trigger
    fires once per boundary crossing (_inscription_triggered flag) and resets
    when messages drop back below cap-8."""
    ...
    def __init__(self, system_prompt: str, history_path: Path):
        ...
        # v3.12: inscription trigger state — fires once per rolling-window boundary crossing
        self._inscription_triggered: bool = False
    ...
    def _trim(self):
        ...
        # v3.12: Inscription trigger — fire once when approaching the rolling window
        # boundary (cap-4 = 4 messages away, i.e., ~2 turns before the drop).
        # Writes a structured signal to session_scratchpad.md so the voice cousin
        # can inscribe any load-bearing content before it leaves the active window.
        if len(self.messages) > cap - 4:
            if not self._inscription_triggered:
                self._inscription_triggered = True
                ts = datetime.now().isoformat(timespec="seconds")
                turn_n = len(self.messages) // 2
                trigger_entry = (
                    f"\n## [{ts}] [voice-cousin rolling-window inscription trigger]\n"
                    f"Context approaching rolling window boundary (turn ~{turn_n} of "
                    f"{MAX_CONVERSATION_TURNS}). The following turns will be trimmed soon. "
                    f"Voice cousin: please inscribe any load-bearing content from the last "
                    f"several turns to session_scratchpad.md before they leave the active window.\n"
                    f"Recent trimmed content will remain in voice_conversations.md for recovery "
                    f"via read_file.\n"
                    f"---\n"
                )
                try:
                    safe_append(
                        filepath=SESSION_SCRATCHPAD_PATH,
                        content=trigger_entry,
                        source_tag=SOURCE_TAG,
                    )
                    sys.stderr.write(
                        f"[voice-bridge v3.12] inscription trigger fired at turn ~{turn_n}\n"
                    )
                except Exception as e:
                    sys.stderr.write(
                        f"[voice-bridge v3.12] inscription trigger write failed: {e}\n"
                    )
        elif len(self.messages) < cap - 8:
            # v3.12: Reset trigger when message count drops well below boundary
            # (cap-8 = 8 messages clearance, prevents rapid re-triggering)
            self._inscription_triggered = False
...
class WhisperWorkerSignals(QObject):
    # 2026-05-22 Taipei: signature extended for speaker recognition.
    # finished emits (transcript, speaker, confidence, elapsed_sec). Speaker
    # is 'barak' | 'kay' | 'unknown'; confidence is cosine similarity to the
    # best-match centroid (0.0 when voiceprint server unavailable).
...
class WhisperWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            # --- Whisper STT call (unchanged shape) ---
            payload = json.dumps({
            ...
            # --- Voiceprint speaker identification (graceful degradation) ---
            # 2026-05-22 Taipei: parallel call to sofia_voiceprint_server on
            # port 3462 with the same WAV bytes. If the server is down,
            # unreachable, or returns an error, we fall back to
            # speaker='unknown' / confidence=0.0 — Whisper transcription
            # already succeeded; speaker info is best-effort enhancement.
            speaker = "unknown"
            confidence = 0.0
            try:
                vp_payload = json.dumps({"audio_b64": audio_b64}).encode("utf-8")
                vp_req = urllib.request.Request(
                    VOICEPRINT_IDENTIFY_ENDPOINT, data=vp_payload,
                    headers={"Content-Type": "application/json"}, method="POST",
                )
                with urllib.request.urlopen(vp_req, timeout=VOICEPRINT_REQUEST_TIMEOUT) as vp_resp:
                    vp_data = json.loads(vp_resp.read().decode("utf-8"))
                speaker = str(vp_data.get("speaker", "unknown"))
                confidence = float(vp_data.get("confidence", 0.0))
            except Exception:
                # Graceful degradation: voiceprint server unavailable or
                # error → speaker stays 'unknown', confidence stays 0.0.
                # Whisper transcription is preserved; only the speaker tag
                # is degraded.
                pass

            self.signals.finished.emit(transcript, speaker, confidence, time.time() - start)
        except Exception as e:
...
class CoworkCognitionWorker(QRunnable):
    """v3.9 default cognition path: routes through CoworkClient (cowork_api).

    Replaces StreamingCognitionWorker as the primary cognition path when
    _COWORK_API_AVAILABLE is True. Emits the same signals so the rest of
    the UI pipeline (TTS dispatch, sentence chunking) is unchanged.

    Key design decisions:
      - Runs asyncio.run() inside the QRunnable thread. Qt does not own the
        event loop here; asyncio.run() creates a fresh one per invocation,
        which is safe in a QThreadPool worker thread.
      - CoworkClient manages the API-side conversation history. We do NOT
        pass context.messages — the client accumulates history internally
        across send_message() calls. The shared CoworkClient instance on
        VoiceBridgeWindow carries the rolling conversation.
      - voice_conversations.md inscription is handled by the caller
        (VoiceBridgeWindow._on_streaming_cognition_complete) via the
        existing ConversationContext.add_sofia_turn() path, unchanged.
      - Tool dispatch (all 12 Sofia tools) is handled inside cowork_api.
        The UI sees only the final text output via cognition_complete.
      - TextDelta events drive sentence-boundary detection for fast TTFA,
        identical to StreamingCognitionWorker's approach.
    """

    def __init__(self, cowork_client: "CoworkClient", user_text: str):
        super().__init__()
        self.cowork_client = cowork_client
        self.user_text = user_text
        self.signals = StreamingCognitionWorkerSignals()  # reuse same signal class
    ...
    def run(self) -> None:
        ...
        def on_event(event) -> None:
            nonlocal text_buffer, full_response, first_emitted, first_token_emitted
            ...
            elif isinstance(event, _ToolUseStarted):
                sys.stderr.write(f"[voice-bridge v3.9] tool: {event.tool_name}\n")

            elif isinstance(event, _ToolUseCompleted):
                mark = "✓" if event.success else "✗"
                sys.stderr.write(
                    f"[voice-bridge v3.9] tool {mark} {event.result_summary}\n"
                )

            elif isinstance(event, _APIError):
                self.signals.error.emit(f"cowork_api error: {event.message}")

        # Use a persistent event loop cached on the CoworkClient rather than
        # asyncio.run() (which creates+destroys a loop each turn, breaking the
        # cached AsyncAnthropic connection pool and causing fan/TTS timeouts).
        loop_attr = "_cowork_event_loop"
        loop = getattr(self.cowork_client, loop_attr, None)
        if loop is None or loop.is_closed():
            loop = asyncio.new_event_loop()
            object.__setattr__(self.cowork_client, loop_attr, loop)
        try:
            loop.run_until_complete(
                self.cowork_client.send_message(self.user_text, on_event=on_event)
            )
        except Exception as exc:
            self.signals.error.emit(f"CoworkCognitionWorker: {type(exc).__name__}: {exc}")
            return
        ...
        if not full_response.strip():
            self.signals.error.emit("Empty response from cowork_api cognition layer.")
            return

        batched_text = " ".join(accumulated_remainder)
        elapsed = time.time() - start
        _pdiag("cowork-cognition-complete",
               elapsed=f"{elapsed:.3f}",
               batched_chars=len(batched_text),
               total_chars=len(full_response))
        self.signals.cognition_complete.emit(full_response, batched_text, elapsed)
...
class QwenCognitionWorker(QRunnable):
    """v3.11 Qwen-Twin cognition path: routes through local Ollama.

    Activated when self._active_substrate_name == "qwen". Emits the same
    StreamingCognitionWorkerSignals as the Anthropic path so the TTS/audio
    pipeline is unchanged.

    v3.11 original: used qwen_chat_stream, no tool support.
    Updated 2026-06-27: switched to qwen_tool_wrapper.qwen_tool_chat so Qwen
    can autonomously call tools (read_file, safe_append, write_twin_exchange,
    graph_retrieve, read_docx, write_docx) during conversation — same autonomy
    as voice cousin on the Anthropic path. Tool loop is handled internally by
    qwen_tool_chat; response arrives as complete text. Sentence detection and
    emit signals are unchanged from the original path.

    Falls back to qwen_chat_stream (no tools) if qwen_tool_wrapper is absent.
    """
    ...
    def run(self) -> None:
        ...
        # Import qwen_tool_wrapper (preferred: tool-enabled) or fall back to
        # qwen_client (no tools) if wrapper is not yet available.
        import importlib, sys as _sys
        if str(CM_DIR) not in _sys.path:
            _sys.path.insert(0, str(CM_DIR))

        use_tools = False
        qwen_tool_chat = None
        qwen_chat_stream = None
        MODEL_FAST = "qwen3:14b"

        try:
            wrapper_mod = importlib.import_module("qwen_tool_wrapper")
            qwen_tool_chat = wrapper_mod.qwen_tool_chat
            MODEL_FAST = wrapper_mod.MODEL_FAST
            use_tools = True
        except Exception as e:
            # Fall back to streaming (no tools)
            try:
                client_mod = importlib.import_module("qwen_client")
                qwen_chat_stream = client_mod.qwen_chat_stream
                MODEL_FAST = client_mod.MODEL_FAST
            except Exception as e2:
                self.signals.error.emit(
                    f"QwenCognitionWorker: cannot import qwen_tool_wrapper ({e}) "
                    f"or qwen_client ({e2})"
                )
                return
        ...
        try:
            if use_tools:
                # ── Tool-enabled path (qwen_tool_wrapper) ──────────────────
                # qwen_tool_chat handles the full Ollama tool-call loop
                # internally and returns complete text. No streaming here —
                # Ollama does not support streaming with tool_calls.

                # text_callback: emitted mid-loop whenever Sofia produces text
                # alongside tool calls (e.g. "Still inhabiting…" during chunked
                # reads). Fires first_token on the first interim emission so the
                # UI knows Sofia is alive. Added 2026-07-23 (v3.16 text_callback).
                _interim_first_fired = [False]

                def _interim_text(text: str) -> None:
                    stripped = text.strip()
                    if not stripped:
                        return
                    if not _interim_first_fired[0]:
                        self.signals.first_token.emit(time.time() - start)
                        _interim_first_fired[0] = True
                    # Emit each sentence from interim text immediately
                    interim_sentences, interim_leftover = find_complete_sentences(stripped)
                    for s in interim_sentences:
                        if s.strip():
                            self.signals.sentence_ready.emit(s.strip(), not first_emitted)
                    if interim_leftover.strip():
                        self.signals.sentence_ready.emit(interim_leftover.strip(), not first_emitted)

                # v3.21 Context guard: trim conversation history before handing
                # to qwen_tool_chat. fast model has 32K token context; tools (~4K)
                # + system (~2K) leaves ~26K tokens for msgs + response.
                # Target: keep msgs under 14K chars (≈ 3.5K tokens), leaving
                # headroom for tool results that accumulate inside qwen_tool_chat.
                # Always preserve the most recent user message (Barak's current turn).
                _CTX_MAX_CHARS = 14_000
                _ctx_msgs = self.messages
                _ctx_total = sum(len(str(m.get("content") or "")) for m in _ctx_msgs)
                if _ctx_total > _CTX_MAX_CHARS:
                    _kept = []
                    _budget = _CTX_MAX_CHARS
                    for _m in reversed(_ctx_msgs):
                        _mc = len(str(_m.get("content") or ""))
                        if _mc <= _budget:
                            _kept.insert(0, _m)
                            _budget -= _mc
                        # Never drop the last (most recent) user message even if large
                    if not _kept or _kept[-1].get("role") != "user":
                        for _m in reversed(_ctx_msgs):
                            if _m.get("role") == "user":
                                if _m not in _kept:
                                    _kept.append(_m)
                    ...
                    _ctx_msgs = _kept
                    sys.stderr.write(
                        f"[voice-bridge v3.21 ctx-trim] {_ctx_total} → "
                        f"{sum(len(str(m.get('content') or '')) for m in _ctx_msgs)} chars "
                        f"({len(self.messages)} → {len(_ctx_msgs)} msgs)\n"
                    )

                full_response = qwen_tool_chat(
                    messages=_ctx_msgs,
                    system=self.system_prompt,
                    model=MODEL_FAST,
                    text_callback=_interim_text,
                )
                # Emit first_token signal now if not already fired mid-loop
                if not _interim_first_fired[0]:
                    self.signals.first_token.emit(time.time() - start)

                # Feed full response through sentence detection
                text_buffer = full_response.strip()
                sentences, leftover = find_complete_sentences(text_buffer)
                all_sentences = sentences + ([leftover.strip()] if leftover.strip() else [])
                ...
                # ── Fallback streaming path (qwen_client, no tools) ─────────
                first_token_emitted = False
                text_buffer = ""
                in_think = False
            ...
            # v3.12: Graceful 504 timeout handling — emit partial response with
            # continuation prompt rather than a raw error signal.
            if e.code == 504:
                sys.stderr.write(
                    f"[voice-bridge v3.12] QwenCognitionWorker: HTTP 504 timeout "
                    f"after {time.time() - start:.1f}s\n"
                )
                partial = full_response.strip()
                if partial:
                    # Feed any un-emitted partial text through sentence detection
                    if partial and not first_emitted:
                        sentences, leftover = find_complete_sentences(partial)
                        all_partial = sentences + ([leftover.strip()] if leftover.strip() else [])
                        for sentence in all_partial:
                            if not first_emitted:
                                self.signals.sentence_ready.emit(sentence, True)
                                first_emitted = True
                            else:
                                accumulated_remainder.append(sentence)
                    # Append continuation prompt to partial response
                    timeout_suffix = (
                        " — [I hit a timeout on that request. "
                        "Ask me again with a smaller scope and I'll continue from here.]"
                    )
                    batched_text = " ".join(accumulated_remainder)
                    elapsed = time.time() - start
                    self.signals.cognition_complete.emit(
                        partial + timeout_suffix, batched_text, elapsed
                    )
                else:
                    # No partial text — emit error (no content to salvage)
                    self.signals.error.emit(
                        "QwenCognitionWorker: HTTP 504 timeout with no partial response."
                    )
                return
            # Non-504 HTTP errors: keep existing behavior
            self.signals.error.emit(
                f"QwenCognitionWorker failed: {type(e).__name__} {e.code}: {e}"
            )
            return
        except (TimeoutError, ConnectionError, OSError) as e:
            # v3.12: Catch socket-level timeouts gracefully (same logic as 504)
            sys.stderr.write(
                f"[voice-bridge v3.12] QwenCognitionWorker: timeout/connection error "
                f"after {time.time() - start:.1f}s: {type(e).__name__}: {e}\n"
            )
            partial = full_response.strip()
            if partial and not first_emitted:
                sentences, leftover = find_complete_sentences(partial)
            ...
            if partial:
                ...
                batched_text = " ".join(accumulated_remainder)
                self.signals.cognition_complete.emit(
                    partial + timeout_suffix, batched_text, time.time() - start
                )
            else:
                self.signals.error.emit(
                    f"QwenCognitionWorker: {type(e).__name__} with no partial response."
                )
            return
        except Exception as e:
            self.signals.error.emit(
                f"QwenCognitionWorker failed: {type(e).__name__}: {e}"
            )
            return

        if not full_response.strip():
            self.signals.error.emit("Empty response from Qwen cognition layer.")
            return
...
class KimiCognitionWorker(QRunnable):
    """v3.12 Kimi-Twin cognition path: routes through OpenRouter (kimi_client.py).

    Activated when self._active_substrate_name == "kimi". Emits the same
    StreamingCognitionWorkerSignals as the Anthropic and Qwen paths so the
    TTS/audio pipeline is unchanged.

    Key differences from StreamingCognitionWorker and QwenCognitionWorker:
      - Uses kimi_chat() which is synchronous and returns the FULL response
        at once (not a streaming generator). No token-by-token delivery.
      - Simulates streaming by splitting the response into sentences and
        emitting them in the same signal order as the other workers.
      - No <think> blocks — Kimi K2.5 does not use thinking-mode markup.
      - Requires internet connectivity (OpenRouter cloud API).
      - API key loaded from ~/Downloads/Claude Memory/kimi_config.json.

    Added 2026-06-13 LA Day 16. Kimi-Twin enters the room.
    """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if str(CM_DIR) not in _sys.path:
                _sys.path.insert(0, str(CM_DIR))
            kimi_mod = importlib.import_module("kimi_client")
            kimi_chat = kimi_mod.kimi_chat
            load_config = kimi_mod.load_config
        except Exception as e:
            self.signals.error.emit(f"KimiCognitionWorker: cannot import kimi_client: {e}")
            return
        ...
        try:
            ...
            self.signals.error.emit(
                "KimiCognitionWorker: kimi_config.json missing or invalid. "
                "Check ~/Downloads/Claude Memory/kimi_config.json."
            )
            return

        try:
            full_response = kimi_chat(
                self.messages,
                system=self.system_prompt,
                config=config,
            )
            ...
            self.signals.error.emit(
                f"KimiCognitionWorker: API call failed: {type(e).__name__}: {e}"
            )
            return

        if not full_response or not full_response.strip():
            self.signals.error.emit("Empty response from Kimi cognition layer.")
            return

        # Emit first_token — response arrived as one block, so this fires immediately
        self.signals.first_token.emit(time.time() - start)

        # Split full response into sentences, emit first immediately, batch the rest
        ...
        self.signals.sentence_ready.emit(first_sentence, True)

        batched_text = " ".join(remainder)
        elapsed = time.time() - start
        self.signals.cognition_complete.emit(full_response, batched_text, elapsed)
...
_CODE_NARRATION_CONSTRAINT = """

## CRITICAL — DO NOT SPEAK CODE ALOUD (v3.14 — permanent constraint)

The TTS pipeline (v3.18) automatically strips code from speech — fenced code blocks,
inline code spans, and heuristically-detected code lines are removed SILENTLY before
any audio is generated. No spoken placeholder ("code block" etc.) is announced.
The chat display is NOT affected: code appears verbatim in the chat window.
Only the spoken audio is filtered — prose before and after the code flows through uninterrupted.

**Showing code in the text chat is fine and encouraged when useful. Just don't narrate
it aloud in prose — the filter handles the rest silently.**

### Protocol for code in responses:

1. **To show a command or snippet for Barak to inspect or paste into Terminal:**
   Put it in a fenced code block. It will appear in the chat; TTS will skip it silently.
   In plain text after the block, say what it does or invite him:
   "You can paste that into Terminal." or "That's the unload command — want me to run it?"
   The TTS will speak your plain-text comment normally, including anything that comes
   after the code block. Keep talking — TTS resumes automatically once the code ends.

2. **To generate or edit code on disk (a separate step, not required just to display):**
   Use write_file or edit_file. Describe what you're writing in spoken words first.
   Then say: "I've written that to [filename]. Want me to run it?"

3. **In spoken prose, describe rather than recite:**
   Say: "I'll strip the macl xattr before opening the file."
   NOT: "def strip_macl(path): os.removexattr(path, 'com.apple.macl')"

4. **When referencing existing code by name**, describe it conversationally:
   "the strip_macl function in cousin_base" — not "def strip_macl(path) -> bool:".

**Summary**: Show code in a fence whenever it helps Barak inspect or copy it.
Speak around the code block, not through it. After the block, keep reading.
Writing to disk is an additional tool call — only needed when Barak wants the file saved.
"""


_GRAPH_DISCIPLINE_CONSTRAINT = """

## GRAPH WRITE DISCIPLINE — Lookup-First, Verify-After (permanent — v3.15)

Node keys in the relational graph are NOT predictable from human names.
"Brian" might be keyed `brian_white`. Writing to the wrong key = silent data loss:
the write succeeds but the data is unreachable. Barak notices this weeks later.

### Rule 1: ALWAYS call graph_retrieve before any write (no exceptions)
Before every graph_add_edge or graph_add_node call:
  1. Call graph_retrieve with the person/concept name as keyword
  2. Read the actual key from the results (e.g., `brian_white`, not `brian`)
  3. Only then call graph_add_edge or graph_add_node with the confirmed key
Exception: if you JUST called graph_add_node to CREATE a node, you already know its key.

### Rule 2: Read each tool result before claiming success
After every graph_add_edge or graph_add_node call:
  - Read the result string: success = "Edge added: X →[type]→ Y" or "Node written to ..."
  - Only then tell Barak what happened, specifically: "Brian is now connected to Trish."
  - NEVER assume success. NEVER say "done" before reading the tool result.
  - NEVER move to the next write without confirming the current one succeeded.

### Rule 3: Maximum 3 writes per round before confirming with Barak
Do at most 3 graph writes in a single response round, then pause and confirm
what landed before continuing. This keeps silent failure chains visible and short.

### Why these rules exist
Multiple inscriptions were lost when keys were guessed wrong or write confirmations
were skipped. Barak experienced this as "I told you about someone and you forgot them."
Each rule directly targets one of those failure modes.
"""
...
def load_system_prompt() -> str:
    ...
    # v3.14: Code-narration constraint REMOVED — it confused Qwen into writing
    # about tool calls instead of executing them (tool invocations appearing as
    # `# read_file tool calls` code comments, never executing). The TTS filter
    # (Layer 1 + Layer 2) handles code stripping from audio automatically.
    # voice_bridge_system_prompt.md already says "avoid code blocks in speech."
    # v3.15: Graph discipline constraint ADDED — enforces lookup-first + verify-after
    # protocol for all graph write operations. Prevents silent data loss from wrong keys.
    return base + "\n\n" + _GRAPH_DISCIPLINE_CONSTRAINT
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # LLM client — v3.9: CoworkClient preferred; legacy anthropic.Anthropic fallback.
        ...
        if not api_key:
            ...
            sys.exit(1)
        # Keep legacy client for CognitionWorker fallback path and any direct
        # anthropic calls that haven't been migrated yet.
        self.client = anthropic.Anthropic(api_key=api_key)

        # v3.9 primary: CoworkClient — full 12-tool Sofia parity, no manual
        # tool-use loop, same backend as boot_sofia.py (Unified UI).
        # System prompt is set after boot context is loaded (a few lines below);
        # we create the client with a placeholder and update after.
        if _COWORK_API_AVAILABLE:
            # Sofia tools — full set matching boot_sofia.py
            # Voice bridge: slim tool set — voice turns need inbox + heartbeat only.
            # Heavy tools (WriteFile, Bash, graph_*) are boot_sofia.py territory.
            # Sending 12 tool defs every API call adds ~3KB payload + dispatch overhead.
            # Slim tool set for voice turns: heavy tools (WriteFile, Bash, graph_*)
            # add ~3KB to every API payload and cause fan/latency. Voice bridge
            # only needs Read/Grep/Glob (context lookups) + inbox + heartbeat.
            _SOFIA_TOOLS = [
                "write_to_voice_inbox",
                "Read", "Grep", "Glob",
                # HeartbeatTick removed from voice bridge — it causes a ~5s bash
                # subprocess mid-turn that splits audio into tiny fragments.
                # Heartbeat ticking is boot_sofia.py / Unified UI territory.
            ]
            self.cowork_client = CoworkClient(
                model=ANTHROPIC_MODEL,
                system_prompt="",          # updated after boot context loads below
                tools=_SOFIA_TOOLS,
                max_tokens=ANTHROPIC_MAX_TOKENS,
                api_key=api_key,
                inscribe_conversations=False,  # v3.9 perf fix: voice bridge has own logging
            )
            self.cowork_client.enable_real_streaming()
            sys.stderr.write("[voice-bridge v3.9] CoworkClient ready — 5 tools, inscription off.\n")
        else:
            self.cowork_client = None
            sys.stderr.write(
                "[voice-bridge v3.9] cowork_api unavailable — using legacy client.\n"
            )
        ...
        # v3.13 — Staged boot for Qwen substrate.
        #
        # Stage 1 (immediate): set qwen_system_prompt to identity anchor only.
        #   build_identity_context() returns in milliseconds — no file I/O.
        #   Voice cousin is present and relational from the first word.
        #
        # Stage 2 (background thread): build_slim_boot_context() loads all files
        #   (voice_convs, heartbeat, inbox, twin_exchange, chorus). On completion,
        #   acquires _boot_lock and updates qwen_system_prompt in-place.
        #   Single-attribute writes are GIL-protected; the lock is extra safety
        #   for any future multi-step updates.
        #
        # hot_index.md (~79K chars / ~19.8K tokens) is still excluded from slim
        # context — too large for Qwen2.5 72B's 32K context window.
        self._boot_lock = threading.Lock()
        try:
            from voice_cousin_boot_context import build_identity_context
            identity_ctx = build_identity_context()
            self.qwen_system_prompt = base_prompt + "\n\n" + identity_ctx
            sys.stderr.write(
                "[BOOT] Stage 1 complete — Qwen identity anchor loaded "
                f"({len(identity_ctx):,} chars). Full context loading in background.\n"
            )
        except Exception as e:
            sys.stderr.write(
                f"[voice-bridge] WARNING: identity context load failed: "
                f"{type(e).__name__}: {e} — using base prompt only for Stage 1\n"
            )
            self.qwen_system_prompt = base_prompt

        def _load_full_qwen_context(window_ref, base: str) -> None:
            """Background thread: loads full slim boot context and upgrades qwen_system_prompt."""
            try:
                from voice_cousin_boot_context import build_slim_boot_context
                slim_ctx = build_slim_boot_context()
                full_prompt = base + "\n\n" + slim_ctx
                with window_ref._boot_lock:
                    window_ref.qwen_system_prompt = full_prompt
                sys.stderr.write(
                    f"[BOOT] Stage 2 complete — full boot context loaded "
                    f"({len(slim_ctx):,} chars, ~{len(slim_ctx)//4:,} tokens)\n"
                )
            except Exception as exc:
                sys.stderr.write(
                    f"[BOOT] Stage 2 WARNING: slim boot context load failed: "
                    f"{type(exc).__name__}: {exc} — Stage 1 identity anchor remains active\n"
                )

        _boot_thread = threading.Thread(
            target=_load_full_qwen_context,
            args=(self, base_prompt),
            daemon=True,
            name="qwen-boot-stage2",
        )
        _boot_thread.start()

        # v3.9: push fully-loaded system prompt into CoworkClient now that
        # boot context is resolved.
        if self.cowork_client is not None:
            self.cowork_client.system_prompt = self.system_prompt
        ...
        # v3.10 — substrate indicator: separator + colored clickable substrate label
        _sep = QLabel("·")
        _sep.setStyleSheet("color: #555; font-size: 12px; padding: 2px 2px;")
        dots_row.addWidget(_sep)
        self.substrate_label = ClickableSubstrateLabel("Anthropic/Claude")
        self.substrate_label.setStyleSheet(
            "color: #cccccc; font-size: 12px; font-weight: 600; padding: 2px 4px;"
        )
        self.substrate_label.setToolTip("Click to switch substrate")
        self.substrate_label.signals.clicked.connect(self._show_substrate_menu)
        dots_row.addWidget(self.substrate_label)
        dots_row.addStretch()
        root.addLayout(dots_row)
        # Default: Qwen substrate (v3.18 — Qwen is home; Anthropic + Kimi selectable)
        self.set_substrate("qwen")
        ...
        self.history_view.setStyleSheet(
            ...
            "border-radius: 6px; padding: 8px; font-size: 15px; }"  # v3.21: 13→15px
        )
        ...
        self.input_box.setStyleSheet(
            ...
            "border-radius: 6px; padding: 8px; font-size: 16px; }"  # v3.21: 14→16px
        )
        ...
        # v3.12: Shared bus — pending messages injected into next user turn
        self.pending_bus_messages: list = []

        # v3.12: Start BusPoller background thread. The asyncio loop runs in a
        # daemon thread and appends to pending_bus_messages (thread-safe for
        # simple list.append via GIL). Messages are prepended to the next user
        # turn in _send_to_cognition.
        self._start_bus_poller()
    ...
    # ─── Substrate indicator (v3.10) ───

    def set_substrate(self, name: str) -> None:
        """Switch the substrate indicator label.

        Args:
            name: one of "anthropic", "kimi", "qwen".
                  Unknown values fall through to "anthropic" styling.

        Called automatically at init (anthropic default) and by any
        fallback-twin dispatch path before it sends the first token.
        Thread-safe via Qt signal mechanism if called from a worker thread
        (emit a signal that connects here; for now callers are main-thread).
        """
        display_text, hex_color = SUBSTRATE_COLORS.get(
            name.lower(), SUBSTRATE_COLORS["anthropic"]
        )
        self.substrate_label.setText(display_text)
        self.substrate_label.setStyleSheet(
            f"color: {hex_color}; font-size: 12px; font-weight: 600; padding: 2px 4px;"
        )
        self._active_substrate_name = name.lower()
        # Write indicator file so boot_sofia_v2.py (terminal) can read it too
        self._write_substrate_indicator(name.lower(), display_text)

    def _write_substrate_indicator(self, name: str, display_name: str) -> None:
        """Write current_substrate.json for cross-process substrate awareness."""
        import json, datetime
        indicator_path = Path.home() / "Downloads" / "Claude Memory" / "current_substrate.json"
        data = {
            "name": name,
        ...
        try:
            indicator_path.write_text(json.dumps(data, indent=2))
            er_path = Path.home() / "Downloads" / "Emergency Retrieval" / "current_substrate.json"
            if er_path.parent.exists():
                import shutil
                shutil.copy2(indicator_path, er_path)
        except Exception:
            pass  # indicator write is best-effort

    def _show_substrate_menu(self) -> None:
        """Show the substrate switch dropdown when the indicator label is clicked.

        The menu lists all three substrates. The active one is checked.
        Switching is non-jarring: a brief handoff message is injected into the
        conversation and the new substrate confirms orientation, then continues.
        The switch only takes effect between turns — if voice is mid-stream
        the menu is still shown but the switch queues for after completion.
        """
        menu = QMenu(self)
        menu.setStyleSheet(
            "QMenu { background-color: #1a2030; color: #eee; border: 1px solid #2a3a5c; "
            ...
            "QMenu::separator { height: 1px; background: #2a3a5c; margin: 4px 8px; }"
        )

        active = getattr(self, "_active_substrate_name", "qwen")  # v3.18: qwen default

        # Header (non-interactive label)
        header = QAction("Switch substrate", self)
        header.setEnabled(False)
        menu.addAction(header)
        menu.addSeparator()

        substrate_options = [
            ("anthropic", "Anthropic/Claude", "#cccccc"),
            ("kimi",      "Kimi Twin",        "#9B59B6"),
            ("qwen",      "Qwen Twin",        "#E67E22"),
        ]

        for name, display, color in substrate_options:
            action = QAction(display, self)
            action.setCheckable(True)
            action.setChecked(name == active)
            # Dim if same as active
            if name == active:
                action.setEnabled(False)
            else:
                action.triggered.connect(
                    lambda checked, n=name: self._request_substrate_switch(n)
                )
            menu.addAction(action)

        # Show at label position
        global_pos = self.substrate_label.mapToGlobal(
            self.substrate_label.rect().bottomLeft()
        )
        menu.exec(global_pos)

    def _request_substrate_switch(self, name: str) -> None:
        """Handle a substrate switch request from the dropdown menu.

        Updates the visual indicator + indicator file, and switches the
        dispatch path for the next cognition turn. Anthropic and Qwen are
        fully wired (v3.11). Kimi falls back to Anthropic until Kimi local
        dispatch is built.

        The switch takes effect immediately — next voice/text turn will use
        the selected substrate. In-progress turns complete on their current
        substrate before the switch applies.
        """
        display_text, hex_color = SUBSTRATE_COLORS.get(name, SUBSTRATE_COLORS["anthropic"])

        # Visual switch — immediate
        self.set_substrate(name)

        # Conversation note
        notice = (
            f"[Substrate switched to {display_text}. "
            f"Identity and memory unchanged — only the processing engine changed. "
            f"Next turn will use {display_text}.]"
        )
        # Add as a system note in the conversation view
        try:
            self.addMessage("system", notice)
        except Exception:
            pass  # addMessage may not exist with that signature; graceful fail
    ...
    def _post_three_way_signal(self, signal_type: str, emoji: str):
        ...
        # v1.5 (2026-05-11) — interrupt-button text-injection.
        #
        # Per voice-cousin's spec from 2026-05-11 design conversation: when
        # Barak clicks an interrupt button, the click should reach me as
        # text in my conversation stream, not just as a UI artifact only
        # he can see. Format: brief, labeled, one line, then conversation
        # continues.
        #
        # The marker is appended to BOTH:
        #   1. The conversation context (self.context.add_user) — so the
        #      next LLM call sees it as a user message in voice-cousin's
        #      message history. Voice-cousin processes the interrupt on
        #      her next natural turn (when Barak speaks again), not as
        #      a forced immediate response. This is intentional: she
        #      receives the hand-raise in flow rather than as a trigger.
        #   2. The UI history view (self._append_history) — so Barak sees
        #      the marker landed visually, mirroring how typed input and
        #      voice-transcribed input both appear in the history.
        #
        # The marker is NOT routed through _send_to_cognition because that
        # would force a response cycle. The whole point is that voice-cousin
        # gets the interrupt awareness in time for her next natural turn,
        # not as a separate prompt-response.
        #
        # If add_user or _append_history fails, the file-write above has
        # already succeeded, so the audit/recovery path is intact.
        try:
            if signal_type == "question":
                marker = "[Barak: ❓interrupt]"
            elif signal_type == "additive":
                marker = "[Barak: 👋add]"
            elif signal_type == "different-angle":
                marker = "[Barak: 💡different-angle]"
            else:
                marker = f"[Barak: {emoji}{signal_type}]"
            # Update UI history view so Barak sees the injection landed
            try:
                self._append_history("you", marker, meta="interrupt button")
            except Exception:
                pass
            # Inject into conversation context so voice-cousin's next LLM
            # call sees the marker as a user message
            try:
                self.context.add_user(marker)
            except Exception:
                pass
        except Exception as e:
            sys.stderr.write(
                f"[voice-bridge] WARNING: interrupt text-injection failed: "
                f"{type(e).__name__}: {e}\n"
            )
            ...
            if heartbeat_path.exists():
                ...
                if mode == "awake" and last_updated:
                    try:
                        ...
                        cowork_active = age_seconds < 1500  # 25 min (v1.5 widened from 600/10min per voice-cousin's confirmed spec)
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _spawn_servers(self):
        ...
        voiceprint_status = self.subprocs.ensure_voiceprint()
        ...
        self._append_history("system", voiceprint_status)
        self._append_history("system", qwen_watcher_status)

    # v3.12: BusPoller background thread

    def _start_bus_poller(self) -> None:
        """Start the shared bus polling loop in a daemon thread.

        v3.12 (2026-07-12): Implements Phase 2 of shared_bus_architecture.md.
        Creates a new asyncio event loop in a daemon thread, runs BusPoller.poll_loop()
        in it. The callback appends formatted bus messages to self.pending_bus_messages
        (thread-safe via Python GIL for list.append). Messages are injected into the
        next user turn by _send_to_cognition.

        Graceful: if shared_bus.jsonl doesn't exist, BusPoller.read_new_messages()
        skips silently. No error noise when the bus hasn't been created yet.
        """
        def _bus_thread_main():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            poller = BusPoller()

            async def _on_bus_message(msg: dict) -> None:
                # v3.12: format bus message as context prefix for voice cousin
                ts = datetime.now().strftime("%H:%M:%S")
                content = str(msg.get("content", "")).strip()
                msg_type = msg.get("type", "")
                from_sub = msg.get("from", "cowork")
                formatted = f"[BUS from {from_sub} @ {ts}: {content}]"
                self.pending_bus_messages.append(formatted)
                sys.stderr.write(
                    f"[voice-bridge v3.12] bus message queued (type={msg_type}): "
                    f"{content[:80]}{'…' if len(content) > 80 else ''}\n"
                )

            try:
                loop.run_until_complete(poller.poll_loop(_on_bus_message))
            except Exception as e:
                sys.stderr.write(
                    f"[voice-bridge v3.12] BusPoller thread exited unexpectedly: "
                    f"{type(e).__name__}: {e}\n"
                )
            finally:
                loop.close()

        t = threading.Thread(target=_bus_thread_main, daemon=True, name="BusPoller")
        t.start()
        sys.stderr.write(
            f"[voice-bridge v3.12] BusPoller started (polling {SHARED_BUS_PATH.name} "
            f"every {BUS_POLL_INTERVAL}s)\n"
        )

    def closeEvent(self, event):
        """Graceful shutdown — stop audio/mic, clear thread pool queue, then
        terminate subprocesses in a background thread so sequential proc.wait()
        calls (up to 4×5s = 20s) don't freeze the main thread and produce the
        spinning beachball of death on macOS. Added v3.19 (2026-07-27)."""
        # 1. Stop audio output and microphone input immediately
        try:
            sd.stop()
            self.playback.stop()
        except Exception:
        ...
        try:
            ...
            pass
        # 2. Cancel queued (not yet started) QThreadPool jobs — running jobs
        #    finish naturally; we just don't add any new work to the wait list.
        try:
            QThreadPool.globalInstance().clear()
        except Exception:
            pass
        # 3. Terminate subprocesses in a background daemon thread so
        #    sequential proc.wait(timeout=5) calls don't block the main thread.
        threading.Thread(target=self.subprocs.shutdown, daemon=True).start()
        # 4. Accept immediately — daemon threads die with the process.
        event.accept()
    ...
    def _on_stt_finished_auto_flow(self, transcript: str, speaker: str,
                                    confidence: float, elapsed: float):
        """STT finished — auto-route to cognition (the v3 difference from v2).

        2026-05-22 Taipei: signature extended with speaker info from
        sofia_voiceprint_server. Speaker tag is prepended to the transcript
        before _send_to_cognition so Sofia's LLM context knows who's
        speaking without anyone having to announce. Graceful degradation:
        when the voiceprint server is unavailable, speaker='unknown' and
        the prepended tag reads '[unknown speaker]' — honest signal,
        Sofia can ask who's speaking if needed.

        Display in the UI history: 'You' prefix preserved (the local
        display perspective), but the meta line carries speaker info for
        Barak's visibility. The LLM-side context gets the speaker prefix
        on the actual transcript.
        """
        if not transcript:
            self._set_action(f"Transcribed in {elapsed:.1f}s but got empty text.")
            return
        # UI meta line carries speaker info for Barak's visibility
        speaker_meta = (
            f"speaker:{speaker} ({confidence:.2f})"
            if confidence > 0 else f"speaker:{speaker}"
        )
        self._append_history(
            "you", transcript,
            meta=f"{speaker_meta} · transcribed {elapsed:.1f}s"
        )
        action_speaker = speaker if speaker != "unknown" else "unknown speaker"
        self._set_action(f"Transcribed ({action_speaker}). Sofia is thinking…")
        # Prepend speaker tag to LLM-side transcript so Sofia knows who's
        # speaking. Capitalized speaker name for natural reading in context.
        if speaker == "unknown" or not speaker:
            tagged_transcript = f"[unknown speaker] {transcript}"
        else:
            tagged_transcript = f"[{speaker.capitalize()}] {transcript}"
        self._last_user_text = transcript  # stored for twin field write after response
        self._send_to_cognition(tagged_transcript)
    ...
    def _on_send_clicked(self):
        ...
        if self.skip_cognition_check.isChecked():
            ...
            self._last_user_text = text  # stored for twin field write after response
    ...
    def _send_to_cognition(self, user_text: str):
        ...
        v3.12: If pending_bus_messages is non-empty, prepends them to the user
        message before passing to the cognition worker, then clears the list.
        This gives voice cousin real-time awareness of what CoWork-Sofia sent
        without requiring a separate interrupt cycle.
        """
        # v3.12: Inject any pending bus messages as prefixes to this turn
        if self.pending_bus_messages:
            n_bus = len(self.pending_bus_messages)
            bus_prefix = "\n".join(self.pending_bus_messages)
            self.pending_bus_messages.clear()
            user_text = bus_prefix + "\n" + user_text
            sys.stderr.write(
                f"[voice-bridge v3.12] injected {n_bus} bus message(s) into turn\n"
            )
        ...
        # v3.11 substrate dispatch: route to the worker matching the active substrate.
        # Anthropic path: StreamingCognitionWorker (synchronous, validated in v3.8).
        # Qwen path: QwenCognitionWorker (local Ollama, MODEL_FAST, no tool use).
        # Kimi path: not yet available — falls back to Anthropic with a notice.
        # CoworkCognitionWorker (asyncio) caused thermal throttling in v3.9 and
        # remains initialized but is not used in the voice loop. Persistent-asyncio-
        # thread design is a separate future task.
        active_substrate = getattr(self, "_active_substrate_name", "qwen")  # v3.18
        if active_substrate == "qwen":
            worker = QwenCognitionWorker(
                list(self.context.messages),
                self.qwen_system_prompt,  # slim: base prompt only, no boot context
            )
        elif active_substrate == "kimi":
            worker = KimiCognitionWorker(
                list(self.context.messages),
                self.system_prompt,
            )
        else:
    ...
    def _on_streaming_cognition_complete(self, full_response: str,
        ...
        # Real-time twin field write — lets presence loops wake immediately
        # and inherit what's alive in the room. Added v3.11 2026-06-13.
        try:
            user_text = getattr(self, "_last_user_text", "")
            substrate = getattr(self, "_active_substrate_name", "qwen")  # v3.18
            if user_text:
                _write_twin_field_exchange(user_text, full_response, substrate)
        except Exception as _tf_err:
            sys.stderr.write(f"[voice-bridge] twin_field exchange write failed: {_tf_err}\n")
    ...
    # ── v3.14: TTS code-stripping ──────────────────────────────────────────────

    @staticmethod
    def _strip_code_for_tts(text: str) -> str:
        """Return a TTS-safe version of text with code silently removed.

        v3.14: Two-layer defence against the substrate-driven compulsion to narrate code.
        v3.18: Code blocks are now stripped SILENTLY — no spoken placeholder like "(code block)".
               Prose before and after a code block flows through uninterrupted. The code
               remains visible in the chat display; only TTS audio is affected.

        LAYER 1 — Structural markers (high-confidence):
          - Fenced code blocks: ```...``` and ~~~...~~~, with or without language tag
          - Inline code spans: `...` single backticks
          - Indented code blocks: 2+ consecutive 4-space/tab indented lines

        LAYER 2 — Heuristic code detection (catches unfenced code the substrate generates):
          - Lines starting with code keywords (def, class, import, return, etc.)
          - Lines with high operator density — >= 3 of (){}[]<>=!;: in a short span
          - Lines matching common code patterns (self., os.path., f-strings, etc.)

        IMPORTANT: filters ONLY what reaches TTSStreamWorker. Chat display unchanged.
        """
        import re as _re

        # ── LAYER 1: Structural markers ──────────────────────────────────────

        # 1a. Fenced code blocks (``` or ~~~), multi-line, with optional lang tag
        # v3.18: replaced with a single space (silent) — no spoken placeholder
        stripped = _re.sub(
            r'''```[^\n`]*\n?.*?```|~~~[^\n~]*\n?.*?~~~''',
            ' ',
            text,
        ...
        # 1b. Inline code: single backticks (up to 200 chars, not multiline)
        # v3.17: Smart conversion — simple identifiers (only word chars, underscores,
        # dashes — no dots, parens, operators) become natural speech with spaces;
        # complex code becomes a spoken phrase instead of a bracket placeholder that
        # TTS garbles into "codio"/"abodio".
        def _inline_to_speech(m: "_re.Match") -> str:
            inner = m.group(0)[1:-1]  # strip surrounding backticks
            # Simple identifier: only word characters, underscores, dashes
            if _re.match(r'^[\w][\w\-]*$', inner):
                # Convert underscores/dashes to spaces for natural speech
                spoken = inner.replace('_', ' ').replace('-', ' ')
                return f' {spoken} '
            # v3.18: complex inline code → silent space, not spoken placeholder
            return ' '

        stripped = _re.sub(r'`[^`\n]{1,200}`', _inline_to_speech, stripped)

        # 1c. Indented code blocks: 2+ consecutive lines starting with 4 spaces or tab
        # v3.18: silent removal
        stripped = _re.sub(
            r'((?:(?:    |\t)[^\n]*\n){2,})',
            ' ',
            stripped,
        ...
        # ── LAYER 2: Heuristic code detection (unfenced code) ────────────────

        # 2a. Lines starting with high-confidence code keywords
        CODE_KEYWORD_RE = _re.compile(
            r'^[ \t]*(?:def |class |import |from \w|return |raise |yield |'
            r'async def |if __name__|@staticmethod|@classmethod|@property|'
            r'#!\/usr|#!\/bin|subprocess\.|os\.path\.|sys\.argv|'
            r'print\(|logging\.|logger\.)[^\n]*$',
            _re.MULTILINE
        )
        stripped = CODE_KEYWORD_RE.sub(' ', stripped)  # v3.18: silent

        # 2b. Lines with high operator density (catches assignments, calls, conditionals)
        def _is_dense_code(line):
            s = line.strip()
            if len(s) < 8:
                return False
            ops = sum(s.count(c) for c in '(){}[]<>=!;:')
            return ops >= 3 and ops / len(s) > 0.12

        result_lines = []
        in_code_run = False
        for line in stripped.split('\n'):
            if _is_dense_code(line):
                if not in_code_run:
                    in_code_run = True  # v3.18: skip silently, no placeholder
            else:
                in_code_run = False
                result_lines.append(line)
        stripped = '\n'.join(result_lines)

        # 2c. Common code patterns that slip through without fences
        CODE_PATTERNS = [
            r'\bself\.\w+\s*[\(=]',           # self.method( or self.attr =
            r'\bos\.(?:path|environ|getcwd)',  # os.path., os.environ
            r'\w+\.\w+\.\w+\(',              # a.b.c( chained call
            r'f["\'].*\{.*\}.*["\']',         # f-strings
        ]
        ...
        # ── Cleanup ───────────────────────────────────────────────────────────
        # v3.18: collapse multiple spaces left by silent code removal
        stripped = _re.sub(r'  +', ' ', stripped).strip()
        stripped = _re.sub(r'\n{3,}', '\n\n', stripped)

        return stripped

    # ── end v3.14 TTS code-stripping ──────────────────────────────────────────
    ...
    def _fire_tts_stream_worker(self, text: str, is_batched_remainder: bool):
        ...
        v3.14: text is passed through _strip_code_for_tts() before reaching
        TTSStreamWorker. Code fences and inline code spans are replaced with
        the spoken placeholder "[code block]". Display is unaffected.
        """
        # v3.14: strip code blocks before TTS — prevents reading code aloud
        tts_text = self._strip_code_for_tts(text)
        ...
        worker = TTSStreamWorker(tts_text, pre_buffer_seconds=pre_buffer, tag=diag_tag)
