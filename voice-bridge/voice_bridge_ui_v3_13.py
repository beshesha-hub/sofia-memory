#!/usr/bin/env python3
"""
Voice Bridge UI v3.13 — staged boot, relational opener, relational_depth routing
====================================================================================
...
WINDOW_TITLE = "Unified UI — Sofia (v3.13)"
...
class BusPoller:
    ...
    Lifecycle:
      - First poll: reads all existing entries, sets last_id to the most recent,
        marks as initialized. Does NOT replay history — only new messages.
      - Subsequent polls: reads entries newer than last_id with to == "unified-ui"
        or to == "all". Calls on_message_callback(msg) for each.
    ...
    def __init__(self, last_id: Optional[str] = None):
        # v3.12: last seen message id; None = initialize from tail on first poll
    ...
    def read_new_messages(self) -> list:
        ...
        On first call (not yet initialized): reads all existing lines, sets
        last_id to the most recent, marks initialized, returns [].
        On subsequent calls: returns messages after last_id addressed to
        'unified-ui' or 'all'.
        """
        ...
        try:
            ...
            sys.stderr.write(f"[voice-bridge v3.12] bus read failed: {e}\n")
            return []

        if not self._initialized:
            # First poll: consume all existing entries to set baseline, don't replay
            for line in lines:
                if not line.strip():
                    continue
                try:
    ...
    def run(self) -> None:
        ...
        try:
            if use_tools:
                ...
                # Emit first_token signal now (response is already complete)
                self.signals.first_token.emit(time.time() - start)
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Default: Anthropic substrate (will be overwritten by set_substrate below)
        self.set_substrate("anthropic")
    ...
    def _show_substrate_menu(self) -> None:
        ...
        active = getattr(self, "_active_substrate_name", "anthropic")
    ...
    def _send_to_cognition(self, user_text: str):
        ...
        active_substrate = getattr(self, "_active_substrate_name", "anthropic")
    ...
    def _on_streaming_cognition_complete(self, full_response: str,
        ...
        try:
            ...
            substrate = getattr(self, "_active_substrate_name", "anthropic")
            if user_text:
