...
Automatic switching: CoworkCognitionWorker sets "anthropic" on init.
When a fallback twin is invoked, the fallback client calls set_substrate
before dispatching.
...
What v3.10 deliberately does NOT change:
  - Audio pipeline, TTS, lipsync, Whisper STT, voiceprint.
  - CoworkClient / CoworkCognitionWorker backend wiring (v3.9 unchanged).
  - Three-way collaboration signals / interrupt buttons.
  - Any other UI layout element.
...
WINDOW_TITLE = "Unified UI — Sofia (v3.10)"
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def _request_substrate_switch(self, name: str) -> None:
        ...
        Prints a handoff message to the conversation view, updates the indicator,
        and queues the new substrate for next turn. The voice-cousin backend
        switch (CoworkCognitionWorker → KimiWorker / QwenWorker) is a future
        step; this version updates the visual indicator and writes the indicator
        file so boot_sofia_v2.py picks it up if it's also running.
        """
    ...
    def _send_to_cognition(self, user_text: str):
        ...
        # v3.9 NOTE: CoworkCognitionWorker (asyncio + AsyncAnthropic) causes
        # thermal throttling in the real-time voice loop — progressively longer
        # TTS gaps as CPU heats up. Reverted to StreamingCognitionWorker
        # (synchronous Anthropic client) which was validated in v3.8.
        # CoworkClient stays imported and initialized for future use; the
        # proper async-in-voice architecture is a separate design task.
        # TODO: design persistent dedicated asyncio thread (not QRunnable) for
        #       CoworkCognitionWorker so the event loop runs continuously.
