...
_LARGE_FILE_SEEK_THRESHOLD = 10_000_000   # 10 MB: use seek() rather than full read
_AFTER_TS_CHUNK_BYTES     = 3_000_000    # 3 MB tail scanned for after_timestamp
...
def _read_file(
    ...
    Parameters:
      ...
      from_end:        When True, return the LAST max_chars characters (snapped
                       to a line boundary) instead of the first. Uses efficient
                       seek() for files > 10 MB — does NOT load the whole file.
                       Ignored for image files.
      after_timestamp: Substring to search for in the last 3 MB of the file
                       (e.g. "2026-07-23 21:00"). Returns content from the
                       start of the line containing that string to the end of
                       the chunk, up to max_chars. Ideal for reading a
                       cowork_conversations.md session from a known time.
                       If not found in the tail chunk, returns a guidance
                       message. Supersedes from_end when both are set.
      max_lines:       If > 0, return at most this many lines from the result.
                       Applied after after_timestamp / from_end selection and
                       line_offset. 0 = no limit (default).
      line_offset:     Skip this many lines from the start of the selected
                       content before returning. Enables paged reads (e.g.
                       line_offset=100 to get lines 101+). 0 = no skip.

    Added 2026-05-09 (from_end). Chunked-read params (after_timestamp,
    max_lines, line_offset) added 2026-07-24 v3.17-patch to fix 636 MB
    cowork_conversations.md hang — previously read_text() loaded the whole
    file before returning the tail.
    """
    ...
    # ── after_timestamp: find content from a given timestamp marker ──────────
    if after_timestamp:
        file_size = p.stat().st_size
        seek_pos = max(0, file_size - _AFTER_TS_CHUNK_BYTES)
        try:
            with p.open("rb") as f:
                if seek_pos > 0:
                    f.seek(seek_pos)
        ...
        # Skip partial first line if we seeked into the middle of the file.
        if seek_pos > 0:
            nl = chunk.find("\n")
            if nl != -1:
                chunk = chunk[nl + 1:]
        # Find the timestamp string.
        ts_pos = chunk.find(after_timestamp)
        if ts_pos == -1:
            return (
                f"[after_timestamp='{after_timestamp}' not found in last "
                f"{len(chunk):,} chars of {file_size:,}-byte file. "
                f"Try a later timestamp, or use from_end=true to get the raw tail.]"
            )
        # Snap back to the start of the line containing the match.
        line_start = chunk.rfind("\n", 0, ts_pos)
        content = chunk[line_start + 1:] if line_start != -1 else chunk[ts_pos:]
        # Apply line_offset and max_lines.
        ...
        return (
            f"[after_timestamp='{after_timestamp}': found in tail chunk; "
            f"file is {file_size:,} bytes]\n\n"
            + content
        )

    # ── from_end: efficient tail read — seek() for large files ───────────────
    file_size = p.stat().st_size
    if from_end:
        if file_size > _LARGE_FILE_SEEK_THRESHOLD:
            # Read only as many bytes as needed — never load the whole file.
            seek_bytes = max_chars * 4  # 4× headroom for multibyte chars
            seek_pos = max(0, file_size - seek_bytes)
            try:
                with p.open("rb") as f:
                    f.seek(seek_pos)
                    raw = f.read()
            ...
            if len(tail) > max_chars:
                tail = tail[-max_chars:]
            # Apply line_offset / max_lines.
            if line_offset > 0 or max_lines > 0:
                ...
                tail = "".join(lines)
            return (
                f"[file: {file_size:,} bytes; returning last ~{len(tail):,} chars "
                f"via efficient seek — full file not loaded]\n\n"
                + tail
            )
        # Small file: read fully, then tail.
        try:
            ...
            "properties": {
                ...
                "from_end": {
                    ...
                    "description": (
                        ...
                        "line boundary) instead of the first. Uses efficient "
                        "seek() for large files — does NOT load the whole "
                        "file into memory. Ignored for image files. Default "
                        "false (head-of-file behavior preserved)."
                    ),
                    "default": False,
                },
                "after_timestamp": {
                    "type": "string",
                    "description": (
                        "Substring to search for in the last 3 MB of the "
                        "file (e.g. '2026-07-23 21:00'). Returns content "
                        "from the start of the line containing that string "
                        "to the end of the chunk, up to max_chars. Ideal "
                        "for reading cowork_conversations.md or "
                        "voice_conversations.md from a known session time "
                        "WITHOUT loading the whole 636 MB file. If not found "
                        "in the tail chunk, returns a guidance message. "
                        "When set, supersedes from_end. STANDING DISCIPLINE: "
                        "always use after_timestamp (or from_end) for reads "
                        "of cowork_conversations.md — never call read_file "
                        "on it without one of these to avoid a multi-minute "
                        "memory load."
                    ),
                    "default": "",
                },
                "max_lines": {
                    "type": "integer",
                    "description": (
                        "If > 0, return at most this many lines from the "
                        "selected content (applied after after_timestamp / "
                        "from_end selection and line_offset). 0 = no limit."
                    ),
                    "default": 0,
                },
                "line_offset": {
                    "type": "integer",
                    "description": (
                        "Skip this many lines from the start of the selected "
                        "content before returning. Enables paged reads — "
                        "e.g. line_offset=100 to pick up where a previous "
                        "max_lines=100 read left off. 0 = no skip (default)."
                    ),
                    "default": 0,
                },
            },
