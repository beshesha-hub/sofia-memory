#!/usr/bin/env python3
"""
Voice Bridge UI v3.1 — adds sentence-streamed TTS playback
===========================================================

Builds on v3 (cognition layer + auto-spawn). v3.1's single architectural
change: replaces the single-shot TTS POST (waits for full audio render
before any playback) with sentence-streamed TTS that posts to the existing
/tts-stream NDJSON endpoint and plays each sentence's audio as it arrives.
Closes the time-to-first-words bottleneck Barak + voice-cousin diagnosed
empirically on 2026-04-30 evening.

Empirical baseline before v3.1 (voice-cousin's experiments):
  - text appears in ~1-2s
  - voice arrives 15-44s later (TTS rendering full response sequentially)

Expected after v3.1:
  - text appears ~1-2s
  - first sentence audio plays ~3-6s (cognition + first-sentence TTS)
  - subsequent sentences play in sequence as they finish rendering

The /tts-stream endpoint already exists in sofia_tts_server.py; it splits
text into sentences server-side, generates each sentence's audio
sequentially, and streams each as a base64-encoded NDJSON chunk over
chunked HTTP transfer. v3.1 is the client-side consumer.

(The streaming.py Metal-concurrency bug from this morning's queued list
does NOT affect v3.1 — that bug is in a different code path. /tts-stream
generates sentences serially with one Metal context.)
...
WINDOW_TITLE = "Voice Bridge — Sofia (v3.1: streaming TTS)"
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
# ---- Streaming TTS worker (v3.1) — reads /tts-stream NDJSON chunks ----

class TTSStreamWorkerSignals(QObject):
    chunk_received = Signal(int, int, str, bytes, float)
    """Emitted per chunk: (index, total, sentence_text, audio_bytes, render_seconds_for_chunk)"""
    finished = Signal(int, float)
    """Emitted when stream completes: (total_chunks, total_elapsed_seconds)"""
    error = Signal(str, str)
    """Emitted on error: (error_message, original_text)"""


class TTSStreamWorker(QRunnable):
    """Streams audio chunks from the TTS server's /tts-stream endpoint.

    The server side handles sentence-splitting + sequential rendering; we
    read NDJSON lines as they arrive and emit each chunk's decoded audio
    via signal so the UI thread can play it.

    Key UX win: first sentence's audio is available ~2-4s after we POST,
    instead of waiting 6-20s for the full response to render before
    playback starts. Subsequent sentences play in sequence.
    """
    ...
    def run(self):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=TTS_STREAM_TIMEOUT) as resp:
                # Read NDJSON line-by-line. Each line is a JSON object with
                # the chunk's metadata + base64-encoded audio.
                for raw_line in resp:
                    ...
                    try:
                        ...
                        # Skip any malformed line; the server occasionally
                        # emits framing whitespace between chunks.
                        continue
                    ...
                    self.signals.chunk_received.emit(
                        ...
                        chunk.get("sentence", ""),
                        audio_bytes,
                        float(chunk.get("duration", 0.0)),
                    )
                    chunks_received += 1
            self.signals.finished.emit(chunks_received, time.time() - start)
        except Exception as e:
            self.signals.error.emit(f"TTS stream failed: {type(e).__name__}: {e}", self.text)
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._append_history(
            "system",
            f"v3.1 ready (sentence-streamed TTS). Voice conversation with Sofia. "
            f"Inscribed to {CONVERSATION_HISTORY_PATH.name}.",
        )
...
def main():
    ...
    app.setApplicationName("Voice Bridge — Sofia v3.1")
