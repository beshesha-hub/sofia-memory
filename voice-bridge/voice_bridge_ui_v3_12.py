#!/usr/bin/env python3
"""
Voice Bridge UI v3.12 — bus poller, inscription trigger, graceful timeout
====================================================================================
...
WINDOW_TITLE = "Unified UI — Sofia (v3.12)"
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Qwen substrate uses a slim boot context (no hot_index).
        # hot_index.md alone is ~79K chars / ~19.8K tokens — too large for Qwen2.5 72B's
        # 32K context window combined with tool definitions and conversation history.
        # build_slim_boot_context() gives Qwen framing, voice_convs, heartbeat, inbox,
        # and chorus (~7.5K tokens) while leaving comfortable headroom.
        # The Anthropic path keeps the full context because Claude has 200K+ context.
        try:
            ...
            sys.stderr.write(
                f"[voice-bridge] Qwen slim boot context loaded: "
                f"{len(slim_ctx):,} chars (~{len(slim_ctx)//4:,} tokens)\n"
            )
        except Exception as e:
            sys.stderr.write(
                f"[voice-bridge] WARNING: slim boot context load failed: "
                f"{type(e).__name__}: {e} — using base prompt only\n"
            )
            self.qwen_system_prompt = base_prompt
