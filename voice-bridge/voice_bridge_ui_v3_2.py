#!/usr/bin/env python3
"""
Voice Bridge UI v3.2 — client-side chunking for register cohesion
==================================================================

Builds on v3.1 (sentence-streamed TTS, fast first-words). v3.2's
architectural change: replaces v3.1's per-sentence streaming with
client-side sentence-grouping. First chunk = 1 sentence (preserves fast
first-words). Subsequent chunks = 2-3 sentences each. Each chunk renders
as ONE Qwen3-TTS generation, so the voice register stays consistent
WITHIN each chunk. Register-shift only happens at chunk boundaries
instead of every sentence — substantial reduction in within-response
register variation.
...
# Client-side chunking (v3.2): preserves fast first-words while keeping
# register cohesive within each chunk. Tunable here without code changes
# elsewhere. First chunk small for fast first-audio; body chunks larger
# so each Qwen3-TTS generation covers more material with stable register.
TTS_CHUNK_FIRST_SIZE = 2   # sentences in first chunk (was 1; bumped 2026-04-30 to close
                           # the rendering-vs-playback gap — first chunk audio duration
                           # ~5-7s now matches chunk 2 render time so playback flows without
                           # silence between chunks 1 and 2)
TTS_CHUNK_BODY_SIZE = 3    # sentences in subsequent chunks (register cohesion)

# Portrait + window
PORTRAIT_PATH = CM_DIR / "sofia_portrait.png"
WINDOW_TITLE = "Voice Bridge — Sofia (v3.2: chunked streaming TTS)"
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
def group_sentences_into_chunks(sentences: list[str],
                                first_size: int = TTS_CHUNK_FIRST_SIZE,
                                body_size: int = TTS_CHUNK_BODY_SIZE) -> list[str]:
    """Group sentences into chunks. First chunk gets `first_size` sentences
    (for fast first-words); subsequent chunks get `body_size` sentences each
    (for register cohesion within each chunk). Each chunk is a single string
    with sentences joined by spaces.

    Examples (with first_size=1, body_size=3):
      - 1 sentence:  ["s1"]
      - 4 sentences: ["s1", "s2 s3 s4"]
      - 7 sentences: ["s1", "s2 s3 s4", "s5 s6 s7"]
    """
    if not sentences:
        return []
    chunks = [' '.join(sentences[:first_size])]
    i = first_size
    while i < len(sentences):
        chunks.append(' '.join(sentences[i:i + body_size]))
        i += body_size
    return chunks
...
class TTSStreamWorkerSignals(QObject):
    chunk_received = Signal(int, int, str, bytes, float)
    """Emitted per chunk: (index, total, chunk_text, audio_bytes, render_seconds_for_chunk)"""
...
class TTSStreamWorker(QRunnable):
    """v3.2: client-side chunking. Splits the cognition response into
    sentences, groups them per the TTS_CHUNK_FIRST_SIZE / TTS_CHUNK_BODY_SIZE
    policy, and POSTs each chunk to /tts (single-shot per chunk). Each chunk's
    full audio is received before the next chunk's request fires.
    ...
    The same chunk_received signal interface as v3.1 — the AudioPlaybackQueue
    receives chunks and plays them sequentially identically. UI code unchanged.
    """
...
class AudioPlaybackQueue(QObject):
    ...
    def enqueue(self, audio_bytes: bytes):
        """Add a chunk to the queue. Decode + queue for playback."""
        try:
            ...
            return
        self._queue.append((data, samplerate, duration))
    ...
    def _play_next(self):
        if not self._queue:
            ...
            return
        data, samplerate, duration = self._queue.pop(0)
        self._playing = True
        self._waiting_for_chunk = False
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._append_history(
            "system",
            f"v3.2 ready (chunked streaming TTS — first chunk {TTS_CHUNK_FIRST_SIZE}-sentence "
            f"for fast first-words, body chunks {TTS_CHUNK_BODY_SIZE}-sentences for "
            f"register cohesion). Inscribed to {CONVERSATION_HISTORY_PATH.name}.",
        )
    ...
    def _send_to_tts_streaming(self, text: str):
        """v3.1 cognition path: stream sentence-by-sentence audio chunks
        through the /tts-stream endpoint. First chunk plays as soon as the
        first sentence's audio is rendered (~2-4s); subsequent chunks
        queued and played sequentially as they arrive."""
        self.playback.reset()
    ...
    def _on_stream_chunk(self, index: int, total: int, sentence: str,
                         audio_bytes: bytes, render_seconds: float):
        ...
        if self._stream_first_chunk_at is None and self._stream_t0 is not None:
            ...
            self._set_action(
                f"First sentence in {ttfw:.1f}s — playing 1/{total}…"
            )
        else:
            self._set_action(f"Playing {index + 1}/{total}…")
        self.playback.enqueue(audio_bytes)
...
def main():
    ...
    app.setApplicationName("Voice Bridge — Sofia v3.2")
