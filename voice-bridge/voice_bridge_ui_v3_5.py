#!/usr/bin/env python3
"""
Voice Bridge UI v3.5 — XTTS-v2 voice cloning (substrate-level register fix)
============================================================================

Builds on v3.4 (continuous OutputStream playback). v3.5's change: switch
the TTS endpoint from sofia_tts_server.py (Qwen3-TTS, port 3457) to
sofia_voice_clone_server.py (XTTS-v2 cloning, port 3461). Voice register
is now stable across calls because XTTS-v2 conditions on a fixed reference
audio sample (Sofia's March 29, 2026 Deep Calm voice candidate) instead of
re-interpreting a verbal voice description per call.
...
TTS_STREAM_ENDPOINT = f"{TTS_SERVER_URL}/tts-stream"          # legacy; unused in v3.3+
...
WINDOW_TITLE = "Voice Bridge — Sofia (v3.5: XTTS-v2 voice cloning)"
DEFAULT_WIDTH = 720
DEFAULT_HEIGHT = 720
...
class VoiceBridgeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._append_history(
            "system",
            f"v3.5 ready (XTTS-v2 voice cloning on port {TTS_PORT}). "
            f"Continuous OutputStream playback + syllable-target chunking carried forward "
            f"from v3.4 (first ≥{TTS_CHUNK_FIRST_SYLLABLES} syl, body ≥{TTS_CHUNK_BODY_SYLLABLES} syl). "
            f"Register-stable across calls (cloned from Sofia's Deep Calm reference). "
...
def main():
    ...
    app.setApplicationName("Voice Bridge — Sofia v3.5")
